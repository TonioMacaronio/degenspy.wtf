...
from solana.rpc.responses import GetTokenSupplyResp, GetTokenAccountsByMintResp
...
class TestSolanaTokenAnalyzer:
            ...
            MockTokenAccountWrapper(
                account=MockTokenAccount(
                    data=MockParsedAccountData(
                        parsed=MockTokenAccountData(
                            info=MockTokenAccountInfo(
                                owner="3Hx8qP7NyYyRF5Lm9KjT2VpQx4YkD6sG8mN1oP2Qr5Ss7",
                                tokenAmount=MockTokenAmount(
    ...
    async def test_get_token_accounts_success(self, analyzer, mock_token_accounts):
        ...
        with patch.object(analyzer.client, 'get_token_accounts_by_mint', return_value=mock_response):
    ...
    async def test_get_token_accounts_failure(self, analyzer):
        ...
        with patch.object(analyzer.client, 'get_token_accounts_by_mint', side_effect=Exception("Network error")):
    ...
    @pytest.mark.asyncio
    async def test_is_program_account_none(self, analyzer):
        """Test program account detection - account doesn't exist"""
        mock_response = MagicMock()
        mock_response.value = None
        
        with patch.object(analyzer.client, 'get_account_info', return_value=mock_response):
            is_program = await analyzer.is_program_account("invalid_address")
            assert is_program is False

    @pytest.mark.asyncio
    async def test_is_program_account_exception(self, analyzer):
        """Test program account detection - exception handling"""
        with patch.object(analyzer.client, 'get_account_info', side_effect=Exception("Network error")):
            is_program = await analyzer.is_program_account("5Q544fKyKrfqJQv2Lmj8m4EZhLLjSeFdRBJ8XXvKK7Rj8")
            assert is_program is False
    ...
    @pytest.mark.asyncio
    async def test_analyze_token_holders_no_accounts(self, analyzer):
        """Test analysis with no token accounts"""
        mock_supply_response = MockTokenSupplyResponse(
            value=MockTokenSupplyValue(amount="1000000", decimals=6, ui_amount=1.0, ui_amount_string="1")
        )
        ...
        with patch.object(analyzer.client, 'get_token_supply', return_value=mock_supply_response), \
            ...
            with pytest.raises(Exception, match="No token accounts found"):
                await analyzer.analyze_token_holders("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")

    @pytest.mark.asyncio
    async def test_analyze_token_holders_malformed_account(self, analyzer):
        """Test analysis with malformed token account data"""
        mock_supply_response = MockTokenSupplyResponse(
            value=MockTokenSupplyValue(amount="1000000", decimals=6, ui_amount=1.0, ui_amount_string="1")
        )
        
        # Create malformed account
        malformed_account = MagicMock()
        malformed_account.account.data.parsed = None  # This will cause an exception
        
        mock_accounts_response = MagicMock()
        mock_accounts_response.value = [malformed_account]
        
        with patch.object(analyzer.client, 'get_token_supply', return_value=mock_supply_response), \
             patch.object(analyzer.client, 'get_token_accounts_by_mint', return_value=mock_accounts_response):
            
            holders = await analyzer.analyze_token_holders("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")
            # Should return empty list due to malformed data
